...
import type { ChatSectionLink } from '../types/content';
...
function takeUnits(text: string, maxCount: number): { taken: string[]; remaining: string } {
...
function estimateBacklog(queue: string[], pending: string): number {
...
type FinishCallback = ((reply: string) => void) | null;
...
export function useAdaptiveStream() {
  ...
  const [sectionLinks, setSectionLinks] = useState<ChatSectionLink[]>([]);

  const queueRef = useRef<string[]>([]);
  const pendingRef = useRef('');
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const finishRef = useRef<FinishCallback>(null);
  ...
  const drainTickRef = useRef<() => void>(() => undefined);
  ...
  drainTickRef.current = () => {
    ...
    const delayMs = reducedMotionRef.current ? 0 : Math.max(16, 32 - backlog * 0.05);
  ...
  const finish = useCallback(
    (reply: string, links: ChatSectionLink[] = [], onFinish?: (reply: string) => void) => {
